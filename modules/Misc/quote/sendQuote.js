...
module.exports = async (msg, id, channel) => {
    ...
    */
    if (
        quoteMsg.embeds.length > 0 &&
        quoteMsg.embeds.find((e) => e.type == "rich")
    )
        return quoteWithEmbed(msg, quoteMsg);
    else return defaultQuote(msg, quoteMsg);
};
...
function quoteWithEmbed(msg, quoteMsg) {
    ...
    let content = `**${quoteMsg.author.tag}** sent this embed ${diffString(
        quoteMsg
    )}`;
    if (quoteMsg.content) content += `\n\n${quoteMsg.content}`;
...
function parseMessageEmbed(quoteObj) {
    ...
    */
    if (quoteObj.description) embed.setDescription(quoteObj.description);

    if (quoteObj.author)
        embed.setAuthor(
            quoteObj.author.name,
            quoteObj.author.iconURL,
            quoteObj.author.url
        );

    if (quoteObj.title) embed.setTitle(quoteObj.title);

    if (quoteObj.thumbnail) embed.setThumbnail(quoteObj.thumbnail.url);

    if (quoteObj.image) embed.setImage(quoteObj.image.url);
...
function defaultQuote(msg, quoteMsg) {
    ...
    if (quoteMsg.attachments.size > 0) {
        ...
        if (attachments.length > 1)
            embed.addField(
                "Message contains more attachments than just one.",
                `A total of ${attachments.length} were found.`
            );
    }
...
function diffString(quoteMsg) {
    ...
    for (const key in timeDiff) {
        ...
        if (timeDiff[key] > 0) {
            ...
            if (timeDiff[key] > 1) return `${timeDiff[key]} ${key} ago`;
            else
