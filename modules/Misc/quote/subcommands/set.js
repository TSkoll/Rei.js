...
async function checkFlags(msg, args) {
    ...
    if (isNaN(args[1])) throw args[1] + " is not a valid message ID.";

    // Check if the user is trying to overwrite subcommands
    if (bannedWords.includes(args[2])) throw "You can't use this tag name!";
    ...
    if (
        await db.ifRowExists("quotes", { guildid: msg.guild.id, name: args[2] })
    )
    ...
    if (quoteMsg.type != "DEFAULT") throw "You can only save users's messages";
}
