...
module.exports = async function(msg, args, webApiKey) {
    ...
    const tagDl = await request.get(`http://localhost:3000/tag/get/${tagName}?k=${webApiKey}&u=${msg.author.id}`);
    ...
    if (tag.file)
        file = await request({ url: `http://localhost:3000/file/get/${tag.file}?k=${webApiKey}&u=${msg.author.id}`, encoding: null});
