...
module.exports = async function(msg, args) {
    ...
    if (rows.length < 1) throw "You haven't saved any tags yet";

    let embed = new Discord.RichEmbed()
        .setTitle(
            `You have ${rows.length} saved ${rows.length == 1 ? "tag" : "tags"}`
        )
        .setColor("BLUE");
    ...
    for (let i = 0; i < rows.length; i++) {
        ...
        if (row.content)
            content =
                row.content.length > 17
                    ? row.content.substring(0, 17) + "..."
                    : row.content;

        description += `**${row.name}**: ${content} ${
            row.imageid ? ":frame_photo:" : ""
        }\n`;
    }
