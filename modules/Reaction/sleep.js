    ...
    if (args) {
      ...
      if (reiEmbedMessage) {
        ...
        if (eDesc && eDesc.includes("sleep")) {
          ...
          const previousUsers = eDesc
            .slice(0, eDesc.indexOf("put") - 1)
            .split("**")
            .filter(el => el != "" && el != ", " && el != " and " && el != " put " && el != " to slee");
          ...
          if (targetUser === user.user.username) {
            await reiEmbedMessage.edit(
              new Discord.MessageEmbed()
                .setColor(embed.hexColor)
                .setImage(embed.image.url)
                .setDescription(
                  `${previousUsers.map(x => `**${x}**`).join(", ")} and **${
                    msg.author.username
                  }** put **${targetUser}** to sleep!`
                )
            );
