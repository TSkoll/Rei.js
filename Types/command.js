...
class Command {
  ...
  checkFlags(msg) {
    ...
    if (this.ownerOnly && !this.checkOwnerOnly(msg)) throw "You're not my master!";

    if (this.nsfw && msg.guild && this.channelIsNSFW(msg)) throw "This command cannot be used outside NSFW channels!";
    ...
    if (this.disallowDM && this.isInDM(msg)) throw "I can't do that in a DM";

    if (this.rateLimit > 0 && !this.checkCooldown(msg))
      throw `Command on cooldown! Please wait for **${Math.round(
        (this.rateLimit - (Date.now() - this.executed.get(msg.author))) / 1000
      )}** more second(s).`;
  }
  ...
  async sendBasicSuccess(message, content) {
    try {
      let msg = await message.channel.send(new Discord.MessageEmbed().setColor("GREEN").setDescription(content));
  ...
  async sendBasicError(message, content) {
    try {
      let msg = await message.channel.send(new Discord.MessageEmbed().setColor("RED").setDescription(content));
  ...
  searchUser(message, query) {
    ...
    if (message.guild && message.guild.available) {
      ...
      const foundUsers = message.guild.members.cache
        .filter(
          x =>
      ...
      const foundNickname = message.guild.members.cache.find(
        x => x.nickname && x.nickname.toLowerCase().includes(normalizedQuery)
      );
      if (foundNickname) return foundNickname;
    }
